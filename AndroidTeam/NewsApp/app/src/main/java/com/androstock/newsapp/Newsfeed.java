...
public class Newsfeed extends AppCompatActivity {
    ...
    "business-insider","espn","financial-times","focus","google-news"};
            ...
            if (xml.length() > 10) { // Just checking if not empty
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        if (((jsonObject.optString(KEY_TITLE).toString()).indexOf("dog") != -1)||((jsonObject.optString(KEY_DESCRIPTION).toString()).indexOf("finance") != -1)) {
                            dataList.add(map);
                        }
                        if(dataList.size()!=0){
                            loader.setVisibility(View.GONE);
